...
build_variant() {
    ...
    PKGS="dialog cryptsetup lvm2 mdadm void-docs-browse xtools-minimal xmirror chrony tmux ufw unzip 7zip xz bluez cups cups-filters cups-pdf samba void-repo-multilib void-repo-nonfree void-repo-multilib-nonfree $A11Y_PKGS $GRUB_PKGS"
    FONTS="font-misc-misc terminus-font dejavu-fonts-ttf noto-fonts-ttf noto-fonts-cjk noto-fonts-emoji fonts-roboto-ttf terminus-font"
    ...
    case $variant in
        ...
        ;;
        xfce*)
            PKGS="$PKGS $XORG_PKGS $XFCE_PKGS lightdm lightdm-gtk-greeter xfce4 gnome-keyring network-manager-applet gvfs-afc gvfs-mtp gvfs-smb udisks2 firefox xfce4-pulseaudio-plugin xfce4-whiskermenu-plugin"
        ...
        ;;
        cinnamon)
            PKGS="$PKGS $XORG_PKGS $CINNAMON_PKGS lightdm lightdm-gtk-greeter cinnamon gnome-keyring colord gnome-terminal gvfs-afc gvfs-mtp gvfs-smb udisks2 firefox"
            SERVICES="$SERVICES dbus lightdm NetworkManager polkitd"
...
EOF
    ...
    if [ "$variant" != base ]; then
        ...
        if [ "$variant" == "cinnamon" ]; then
            cp -rf include_cinnamon/etc $INCLUDEDIR/
            cp -rf include_cinnamon/usr $INCLUDEDIR/
        fi
        if [ "$variant" == "xfce" ]; then
